...
from app.services.historical_loader import load_all_historical_data
...
# ---------------------------------------------------------------------------
# Setup automático: carga datos históricos + entrena el modelo
# ---------------------------------------------------------------------------

@router.post("/admin/setup")
@router.get("/admin/setup")
async def initial_setup(db: Session = Depends(get_db)):
    """
    Carga datos históricos desde football-data.co.uk + entrena modelo Dixon-Coles
    + genera predicciones iniciales. Llamar una sola vez desde el navegador.
    
    Devuelve resumen de lo que cargó.
    """
    # 1. Cargar datos históricos
    historical = await load_all_historical_data(db)
    
    # 2. Si hay suficientes datos, ajustar el modelo
    model_info = None
    try:
        params = fit_and_cache(db, min_matches=50)
        model_info = {
            "fitted": True,
            "matches_used": params.matches_used,
            "n_teams": len(params.teams),
        ...
        model_info = {"fitted": False, "reason": str(e)}
